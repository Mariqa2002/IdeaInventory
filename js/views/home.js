...
export function renderHome(mount, { rerender }) {
  ...
  if (!ideas.length) {
    page.appendChild(
      emptyState({
        ...
        message:
          "Every project starts as a half-formed thought. Put the first one somewhere safe.",
        art: "chest",
        action: h(
  ...
  page.append(
    ringRow(ideas),
    toolbar(ideas, settings, rerender),
    catalog(ideas, settings, rerender),
  );

  page.appendChild(
    h(
      "button.btn.btn-primary.btn-icon.fab",
      {
        type: "button",
...
function ringRow(ideas) {
  ...
  return h(
    ...
    ringCard({
      ...
      note: total
        ? `${Math.round((done / total) * 100)}% of the inventory`
        : "—",
      color: "var(--ok)",
    }),
    ringCard({
      ...
      note: tasksTotal
        ? `${plural(tasksTotal - tasksDone, "task")} left across every idea`
        : "No tasks added yet",
      color: "var(--info)",
    }),
  );
...
function toolbar(ideas, settings, rerender) {
  const setView = (view) => {
    store.setSetting("ideaView", view);
    rerender();
  };

  const segmented = h(
    "div.segmented",
    { role: "group", "aria-label": "Catalog layout" },
    h(
      "button",
      {
        type: "button",
        "aria-pressed": String(settings.ideaView === "grid"),
        onClick: () => setView("grid"),
      },
      icon("grid"),
      h("span", { text: "Square view" }),
    ),
    h(
      "button",
      {
        type: "button",
        "aria-pressed": String(settings.ideaView === "list"),
        onClick: () => setView("list"),
      },
      icon("list"),
      h("span", { text: "List view" }),
    ),
  );
  ...
  return h(
    ...
    h(
      "div.stack.gap-4",
      h("div.eyebrow", {
        text: plural(ideas.length, "idea") + " in the chest",
      }),
    ...
    h(
      ...
      segmented,
      h(
        "button.btn.btn-primary.hide-phone",
        {
          type: "button",
          onClick: () => openIdeaForm({ onSaved: rerender }),
        },
...
function catalog(ideas, settings, rerender) {
  const list = h(
    "div.catalog.view-" + (settings.ideaView === "list" ? "list" : "grid"),
  );
  for (const idea of ideas)
    list.appendChild(ideaCard(idea, settings, rerender));
  return list;
}
...
function ideaCard(idea, settings, rerender) {
  ...
  const menuButton = h(
    ...
    {
      type: "button",
      "aria-label": `Options for ${idea.title}`,
      onClick: (event) => {
        ...
        openMenu(menuButton, [
          { label: "Open", icon: "chevronRight", onSelect: open },
          {
            label: "Edit details",
            icon: "edit",
            onSelect: () => openIdeaForm({ idea, onSaved: rerender }),
          },
          {
            label: isDone ? "Mark as active" : "Mark as completed",
            icon: isDone ? "refresh" : "checkCircle",
            onSelect: () => {
              store.setIdeaStatus(idea.id, isDone ? "active" : "completed");
              rerender();
            },
          },
          "divider",
  ...
  const badges = h(
    ...
    stats.total
      ? frag(
          h(
            "div.mini-bar",
            { title: `${stats.done} of ${stats.total} tasks completed` },
    ...
    missing
      ? h(
          "span.chip.chip-warn",
          { title: "Long-form questions still unanswered" },
          icon("alert"),
          String(missing),
        )
      : null,
  ...
  if (idea.priority === 1)
    card.style.setProperty("--accent", "var(--plum-700)");
  else if (idea.priority != null)
    card.style.setProperty("--accent", "var(--plum-400)");
